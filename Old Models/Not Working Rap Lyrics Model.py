...
def build_model(vocab_size, embedding_dim, rnn_units, batch_size):
    return tf.keras.Sequential([
        tf.keras.layers.Embedding(vocab_size, embedding_dim, batch_input_shape=[segment_length, None]),
...
def generate_text(model, start_string, num_chars_generate, temperature):
    ...
    input_eval = tf.reshape(input_eval, [40, 1])
    text_generated = []
...
if __name__ == '__main__':
    ...
    artist_name = name
    EPOCHS = 1
    temperature = 1.1  # Low means more predictable results, high means less
    num_chars_generate = 800
    ...
    segment_length = 40
    window_length = segment_length + 1
    examples_per_epoch = len(text) // (segment_length + 1)

    char_dataset = tf.data.Dataset.from_tensor_slices(text_as_int)

    char_dataset = char_dataset.window(window_length, shift=1, drop_remainder=True)

    sequences = char_dataset.flat_map(lambda window: window.batch(window_length))
    # sequences = char_dataset.batch(segment_length + 1, drop_remainder=True)

    dataset = sequences.shuffle(BUFFER_SIZE).batch(BATCH_SIZE)
    ...
    checkpoint_dir = f'{name}_new_training_checkpoints'
    ...
    padding = [" "] * (segment_length - len(start_string))
    start_string = "".join(padding) + start_string
